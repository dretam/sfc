...
if (isset($_POST["addlayanan"])) {
	...
	$path = "../layanan/" . $random . '.' . $ext;
	$pathdb = "layanan/" . $random . '.' . $ext;
	...
	if ($tipe_file == "image/jpeg" || $tipe_file == "image/png") {
		if ($ukuran_file <= 5000000) {
			if (move_uploaded_file($tmp_file, $path)) {
				...
				$query = "insert into layanan (idkategori, namalayanan, gambar, deskripsi, hargabefore, hargaafter)
			  values('$idkategori','$namalayanan','$pathdb','$deskripsi','$hargabefore','$hargaafter')";
				$sql = mysqli_query($conn, $query); // Eksekusi/ Jalankan query dari variabel $query

				if ($sql) {
					echo "<br><meta http-equiv='refresh' content='2; URL=layanan.php'> You will be redirected to the form in 2 seconds";
				} else {
					// Jika Gagal, Lakukan :
...
if (isset($_POST["hapus"])) {
	$kode = $_POST['idlayanan'];
	$query2 = mysqli_query($conn, "delete from layanan where idlayanan='$kode'");
	if ($query2) {
...
<head>
	...
	<title>Manage Service - SFC</title>
						...
						<div class="card">
							<div class="card-body">
								<div class="d-sm-flex justify-content-between align-items-center">
									...
									<button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-info col-md-2">Tambah Layanan</button>
								</div>
								<div class="data-tables datatable-dark">
									<table id="dataTable3" class="display" style="width:100%">
										...
										<tbody>
											<?php
											$brgs = mysqli_query($conn, "SELECT * from kategori k, layanan l where k.idkategori=l.idkategori order by idlayanan ASC");
											$no = 1;
											while ($p = mysqli_fetch_array($brgs) ?? null) {
											...
											?>
												...
												<tr style="text-align: center;">
													<form method="post">
														...
														<td>
															<input type="hidden" name="idlayanan" value="<?php echo $p['idlayanan'] ?>" \>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="layanan.php" method="post" enctype="multipart/form-data">
						<div class="form-group">
							...
							<input name="namalayanan" type="text" class="form-control" required autofocus>
						</div>
						<div class="form-group">
				...
				<div class="modal-footer">
					...
					<input name="addlayanan" type="submit" class="btn btn-primary" value="Tambah">
				</div>
				</form>
			</div>
		</div>
